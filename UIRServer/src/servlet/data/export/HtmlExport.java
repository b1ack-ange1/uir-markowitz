	...
	protected final void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		for (int i = 0; i < metaData.size(); i++) {
			if (req.getParameterMap().containsKey(metaData.get(i))) {
				String filterValue = req.getParameter(metaData.get(i));
				filter.put(i, filterValue);
			}
		}
				...
				for (TRSDSColumnList dsCol : columnList)
					if (dsCol.getDSColumnCode().equalsIgnoreCase(colCode)) {
						...
						if (dsCol.getDSColumnViewAlignment() != null) {
							...
							else if (dsCol.getDSColumnViewAlignment()
									.intValue() == RsdhConstants.cnViewAlignmentCenter)
				...
				if (Data.filter(data.get(rowNum), filter)) {
					...
					for (Map<String, Object> exportCol : exportCols) {
						...
						if (exportCol.get("aligment") != null) {
							stringWriter.append("align='"
									+ exportCol.get("aligment") + "' ");
						}
						stringWriter.append(">");
		...
		writer.write("<BODY><TABLE BORDER='1' CELLSPACING='0'>");
