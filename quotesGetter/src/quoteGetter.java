...
public class quoteGetter {	
	...
	private void getQuote(int tickerID, String tickerName, Calendar calend) {
		try {
			...
			// формируем запрос
			// непон€тный параметр. наверно типа защита от автозапросов :)
			String requestText = "d=d";
			// выбираем акции ћћ¬Ѕ
			requestText += "&market=1";
			// выбираем √азпром
			requestText += "&em="+tickerID;
			// дата, мес€ц (начина€ с 0), год даты начала запроса
			...
			// дата, мес€ц (начина€ с 0), год даты конца запроса
			...
			// периодичность 1 час
			requestText += "&p=7";
			// им€ и расширение получаемого файла
			...
			requestText += "&MSOR=0";
			// разделитель полей зап€та€
			requestText += "&sep=1";
			// разделитель разр€дов пустой
			requestText += "&sep2=1";
			// формат записи в файл: все подр€д
			requestText += "&datf=1";
			// не добавл€ть заголовок файла
			requestText += "&at=0";
			...
			try {
				//—оздаем файл дл€ скидывани€ информации с флагом append
