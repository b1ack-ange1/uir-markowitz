...
public class BPTMaxWealth extends BPTAbstract{
	...
	public static BPTMaxWealth getInstance(int portfolioID, double securityLevel, double failingProbability){
		BPTMaxWealth inst = BPTMaxWealth._instances.get(portfolioID);
		if (inst == null){
			BPTMaxWealth._instances.put(portfolioID, new BPTMaxWealth(securityLevel, failingProbability));
		}
		else if ((inst.getSecurityLevel() != securityLevel) || (inst.getFailingProbability() != failingProbability)){
			BPTMaxWealth._instances.put(portfolioID, new BPTMaxWealth(securityLevel, failingProbability));
		}
